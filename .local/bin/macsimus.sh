...
if [[ ! $(brew --version) ]]; then
  ...
  brew install awscli bash bash-completion coreutils ctags direnv ffmpeg findutils fzf gcc gh git git-lfs gnu-sed gnutls gpg grep htop hugo imagemagick jq lazydocker nodejs openssl p7zip packer pango pipenv pyenv pyenv-virtualenv rbenv rename ripgrep scc stow tmux urlview vim z zlib
