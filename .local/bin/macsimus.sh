...
if [[ ! $(brew --version) ]]; then
  ...
  brew install awscli bash bash-completion coreutils ctags direnv ffmpeg findutils fzf gcc git gnu-sed gnutls golang gpg grep htop httpie hub imagemagick jq mysql nodejs openssl p7zip packer pssh pyenv pyenv-virtualenv rbenv rename ripgrep stow tmux urlview vim z zlib
