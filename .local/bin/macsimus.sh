...
if [[ ! $(brew --version) ]]; then
  ...
  brew install awscli bash bash-completion coreutils ctags direnv ffmpeg findutils fzf gcc git gnu-sed gnutls gpg grep htop httpie hub hugo imagemagick jq mysql nodejs openssl p7zip packer pango pipenv pyenv pyenv-virtualenv rbenv rename ripgrep stow tmux urlview vim z zlib
